"""
seed_demo.py — Creates a complete FlowTeam demo company called "Nova Agency"
...
Usage:
    ...
    python manage.py seed_demo --reset   # drops all Nova Agency data first
"""
...
USERS = [
    # (full_name,              email,                         password,       role in company)
    ("Sarah Chen",             "sarah@nova-agency.com",       "Demo1234!",    "ceo"),
    ("Alex Rivera",            "alex@nova-agency.com",        "Demo1234!",    "admin"),
    ("Priya Sharma",           "priya@nova-agency.com",       "Demo1234!",    "manager"),
    ("Jordan Kim",             "jordan@nova-agency.com",      "Demo1234!",    "member"),
    ("Dana Osei",              "dana@nova-agency.com",        "Demo1234!",    "member"),
    ("Marcus Liu",             "marcus@nova-agency.com",      "Demo1234!",    "viewer"),
]

COMPANY_NAME   = "Nova Agency"
COMPANY_DOMAIN = "nova-agency.com"


class Command(BaseCommand):
    help = "Seed a complete Nova Agency demo company with hierarchy, teams, projects, tasks, and messages."
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            self._create_pending_invite(company, users)
    ...
    def _reset(self):
        ...
        Team.objects.filter(name__in=["Engineering", "Design", "Marketing"]).delete()
        User.objects.filter(email__endswith="@nova-agency.com").delete()
    ...
    def _create_users(self) -> dict[str, User]:
        users = {}
        for full_name, email, password, _role in USERS:
    ...
    def _create_company(self, users: dict) -> "Company":
        ...
        company, _ = Company.objects.get_or_create(
            ...
            defaults={
                ...
                "email_domain_verification_token": "demo-token-abc123",
                "notes": "Demo company seeded by seed_demo management command.",
                "settings_json": {
                    "ai_enabled": True,
        ...
        # Assign company-level roles to every demo user
        role_map = {email: role for _, email, _, role in USERS}
        ...
        # Add one pending invite to show the Invites tab in action
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # Projects, columns, labels, tasks, sprints, milestones
    # ──────────────────────────────────────────────────────────────────────────
    ...
    def _create_tasks_and_sprints(self, projects: dict, users: dict) -> None:
        ...
        sprint_wr, _ = Sprint.objects.get_or_create(
            ...
            defaults={
                "goal": "Ship homepage redesign and new navigation before May 24 deadline.",
        ...
        for user, hours in [(priya, 40), (jordan, 30), (dana, 30), (alex, 20)]:
            SprintCapacity.objects.get_or_create(
                ...
                defaults={"capacity_hours": hours, "notes": ""},
            )
        ...
        # Tasks
        task_data = [
            # (title, col, assignee, priority, issue_type, start_offset, due_offset, labels, description)
            (
                "Design hero section",
                "Done", priya, "high", "story", -12, -2,
                ["design", "frontend"],
                "Create the above-the-fold hero with headline, CTA, and background illustration. Mobile-first.",
            ),
            (
                "Implement hero animation",
                "In Progress", jordan, "high", "task", -5, 3,
                ["frontend", "Q2"],
                "CSS keyframe animation for the hero headline entrance. Must respect prefers-reduced-motion.",
            ),
            (
                "Build pricing page",
                "In Progress", dana, "urgent", "story", -3, 5,
                ["frontend", "Q2"],
                "Three-tier pricing table with toggle (monthly/annual). Stripe checkout links.",
            ),
            (
                "Fix mobile nav dropdown iOS",
                "In Progress", jordan, "urgent", "bug", -2, 2,
                ["bug", "frontend"],
                "Dropdown fails to close on outside tap — iOS Safari 16+. Affects iPhone 12 and above.",
            ),
            (
                "API endpoints for contact form",
                "Dev Review", alex, "normal", "task", -6, -1,
                ["backend"],
                "POST /api/contact/ with reCAPTCHA validation, spam filter, and email notification.",
            ),
            (
                "Homepage copy — final pass",
                "Design Review", priya, "normal", "task", -4, 6,
                ["design", "Q2"],
                "Review all headline and body copy against brand voice guidelines.",
            ),
            (
                "SEO metadata — all pages",
                "Backlog", dana, "high", "task", None, 8,
                ["Q2"],
                "Title tags, OG images, meta descriptions, canonical URLs for all 12 pages.",
            ),
            (
                "Accessibility audit",
                "Backlog", None, "normal", "task", None, 14,
                ["frontend"],
                "WCAG 2.1 AA compliance pass. Run axe-core, fix contrast and focus issues.",
            ),
            (
                "Performance optimisation",
                "Backlog", jordan, "low", "task", None, 18,
                ["frontend", "Q2"],
                "Lighthouse score ≥ 90. Lazy load images, split chunks, CDN for assets.",
            ),
            (
                "Blog section — design",
                "Backlog", priya, "normal", "story", None, 20,
                ["design"],
        ...
        wr_tasks = {}
        for (title, col_name, assignee, priority, itype, s_off, d_off, lbl_names, desc) in task_data:
            col = wr_col.get(col_name) or wr_col.get("Backlog")
            ...
            task, _ = Task.objects.get_or_create(
                ...
                defaults={
                    ...
                    "start_date": start_d,
                    "due_date": due_d,
                    "sprint": sprint_wr if col_name not in ("Backlog",) else None,
        ...
        # Task dependency: "Fix mobile nav" blocked by "Implement hero animation"
        hero_task = wr_tasks.get("Implement hero animation")
        nav_task  = wr_tasks.get("Fix mobile nav dropdown iOS")
        if hero_task and nav_task:
            TaskLink.objects.get_or_create(
                source_task=nav_task,
                target_task=hero_task,
        ...
        for (title, col_name, assignee, priority, itype, s_off, d_off, lbl_names) in mob_task_data:
            col = mob_col.get(col_name) or mob_col.get("Backlog")
            start_d = (today + datetime.timedelta(days=s_off)) if s_off is not None else None
            due_d   = (today + datetime.timedelta(days=d_off)) if d_off is not None else None
            task, _ = Task.objects.get_or_create(
                project=mob,
                ...
                defaults={
                    ...
                    "start_date": start_d,
                    "due_date": due_d,
                    "sprint": sprint_mob if col_name not in ("Backlog",) else None,
        ...
        Milestone.objects.get_or_create(
            project=br,
            name="Brand Guidelines Delivered",
            defaults={
        ...
        for (title, col_name, assignee, priority, itype, s_off, d_off, lbl_names) in br_task_data:
            col = br_col.get(col_name) or br_col.get("Brief")
            start_d = (today + datetime.timedelta(days=s_off)) if s_off is not None else None
            due_d   = (today + datetime.timedelta(days=d_off)) if d_off is not None else None
            task, _ = Task.objects.get_or_create(
                project=br,
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # Messages (channels + seed messages)
    # ──────────────────────────────────────────────────────────────────────────
    ...
    def _create_messages(self, teams: dict, users: dict) -> None:
        ...
        channel_specs = [
            ...
            },
            {
                ...
                "messages": [
                    ...
                    (jordan, "The iOS nav bug is reproducible on Safari 16.4. It's a touch event listener issue — working on a fix."),
                    (alex,   "Decision: we're going with REST for the mobile API — not GraphQL. @Jordan can we update the docs?"),
            ...
            },
            {
                ...
                "messages": [
                    (priya,  "Hero section v3 is in Figma: https://figma.com/demo-link — please review by tomorrow."),
                    ...
                    (dana,   "Pricing page mockup ready: https://figma.com/pricing-demo — two variants, toggle included."),
        ...
        for spec in channel_specs:
            ...
            # Seed messages (only if channel has no messages yet)
            if not channel.messages.exists():
                base_time = timezone.now() - datetime.timedelta(hours=len(spec["messages"]) + 2)
                for i, (sender, text) in enumerate(spec["messages"]):
                    msg_time = base_time + datetime.timedelta(minutes=i * 18)
                    ...
                    # Manually set created_at isn't possible after auto_now_add,
                    # but messages will still show as recent history.

        self.stdout.write("  Channels and messages created.")

    def _create_pending_invite(self, company: "Company", users: dict) -> None:
        """Already created in _create_company; this is a no-op placeholder."""
        pass
    ...
    def _print_summary(self, users: dict) -> None:
        self.stdout.write("\n")
        self.stdout.write(self.style.SUCCESS("=" * 62))
        self.stdout.write(self.style.SUCCESS("  Nova Agency demo data seeded successfully"))
        self.stdout.write(self.style.SUCCESS("=" * 62))
        self.stdout.write("")
        self.stdout.write("  DEMO LOGIN CREDENTIALS")
        self.stdout.write("  " + "-" * 50)
        self.stdout.write(f"  {'Name':<20} {'Email':<32} {'Role':<10}")
        self.stdout.write("  " + "-" * 50)
        for full_name, email, password, role in USERS:
            self.stdout.write(f"  {full_name:<20} {email:<32} {role:<10}")
        self.stdout.write("")
        self.stdout.write("  Password for ALL accounts: Demo1234!")
        self.stdout.write("")
        self.stdout.write("  WHAT'S BEEN CREATED")
        self.stdout.write("  " + "-" * 50)
        self.stdout.write("  Company     : Nova Agency (active, AI plan, domain verified)")
        ...
        self.stdout.write("  Sprints     : Sprint 3 (active) in Website Redesign")
        self.stdout.write("                Sprint 1 (active) in Mobile App")
        self.stdout.write("  Milestones  : Public Beta Launch / SEO Handoff / App Store Submission")
        self.stdout.write("                Brand Guidelines Delivered")
        self.stdout.write("  Tasks       : 26 tasks across 3 projects (all states + priorities)")
        ...
        self.stdout.write("")
        self.stdout.write("  DEMO FLOW SUGGESTIONS")
        self.stdout.write("  " + "-" * 50)
        self.stdout.write("  1. Log in as sarah@nova-agency.com (CEO) - show company dashboard")
        self.stdout.write("  2. Show Invites tab - pending invite from Alex visible")
        self.stdout.write("  3. Switch to alex@nova-agency.com (Admin) - show role diff")
        self.stdout.write("  4. Switch to priya@nova-agency.com (Manager) - show invite scoping")
        self.stdout.write("  5. Open Website Redesign - Board - show all task states")
        self.stdout.write("  6. Open Sprint 3 - Capacity - show per-member hours")
        self.stdout.write("  7. Open #engineering - show thread, decision, PR reference")
        self.stdout.write("  8. Open Super Admin - Company Management - show Nova Agency card")
        self.stdout.write(self.style.SUCCESS("=" * 62))
        self.stdout.write("")
