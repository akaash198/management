...
class TeamActivityConsumer(AsyncWebsocketConsumer):
    """
    Consumer for team-wide activity updates (dashboard feed).
    """
    ...
    async def connect(self):
        ...
        self.user = self.scope["user"]
        if not await self.is_team_member(self.user.id, self.team_id):
            await self.close(code=4001)
            return

        self.group_name = f"team_activity_{self.team_id}"
        await self.channel_layer.group_add(self.group_name, self.channel_name)
        await self.accept()
    ...
    async def activity_update(self, event):
        """
        Receives activity.new event from channel layer and sends to client.
        """
        await self.send(
            text_data=json.dumps(
                {
